...
public class dirthousegenerator {
  private static final Identifier SE_TEMPLATE = new Identifier(struc.MOD_ID + ":houses/dirt");
  ...
  public static void addPieces(StructureManager manager, BlockPos pos, BlockRotation rotation,
    ...
    pieces.add(new dirthousegenerator.Piece(manager, SE_TEMPLATE, pos, rotation));
  ...
  public static class Piece extends SimpleStructurePiece {
    ...
    public Piece(StructureManager manager, Identifier identifier, BlockPos pos, BlockRotation rotation) {
      super(featuring.DIRTHOUSE_PIECES, 0);
      this.template = identifier;
      BlockPos blockPos = (BlockPos) dirthousegenerator.COUNTER_OFFSET.get(identifier);
    ...
    public Piece(StructureManager manager, CompoundTag tag) {
      super(featuring.DIRTHOUSE_PIECES, tag);
    ...
    private void initializeStructureData(StructureManager manager) {
      ...
      StructurePlacementData structurePlacementData = (new StructurePlacementData()).setRotation(this.rotation)
          .setMirrored(BlockMirror.NONE).setPosition((BlockPos) dirthousegenerator.PIECES_OFFSET.get(this.template))
    ...
    protected void handleMetadata(String metadata, BlockPos pos, IWorld world, Random random, BlockBox boundingBox) {
      // BlockPos cheasy = new BlockPos(pos.getX() + 2, pos.getY() + 1, pos.getZ() +
      // 13);
      // if ("chest".equals(metadata)) {
      // world.setBlockState(pos, Blocks.AIR.getDefaultState(), 3);
      // BlockEntity blockEntity = world.getBlockEntity(cheasy);
      // if (blockEntity instanceof ChestBlockEntity) {
      // ((ChestBlockEntity)
      // blockEntity).setLootTable(LootTables.SIMPLE_DUNGEON_CHEST,
      // random.nextLong());
      // }
      // }
    }
    ...
    public boolean generate(IWorld world, ChunkGenerator<?> generator, Random random, BlockBox box, ChunkPos pos) {
      ...
      if ((world.getBlockState(dirt.up()).isAir() || world.getBlockState(dirt.up()).getBlock().equals(Blocks.GRASS))
          ...
          && world.getBlockState(dirt.up(5)).isAir() && world.getBlockState(dirt.south(16).up(5)).isAir()
          && world.getBlockState(dirt.east(16).up(5)).isAir()
          && world.getBlockState(dirt.south(16).east(16).up(5)).isAir()) {

        StructurePlacementData structurePlacementData = (new StructurePlacementData()).setRotation(this.rotation)
            .setMirrored(BlockMirror.NONE).setPosition((BlockPos) dirthousegenerator.PIECES_OFFSET.get(this.template))
            .addProcessor(BlockIgnoreStructureProcessor.IGNORE_STRUCTURE_BLOCKS);
        BlockPos blockPos = (BlockPos) dirthousegenerator.COUNTER_OFFSET.get(this.template);
