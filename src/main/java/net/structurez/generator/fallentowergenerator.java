...
public class fallentowergenerator {
  private static final Identifier TOWER = new Identifier(struc.MOD_ID + ":stuff/fallentower");
  private static final Identifier towerloot = new Identifier("structurez:fallentower_loot");

  public static void addPieces(StructureManager manager, BlockPos pos, BlockRotation rotation,
      List<StructurePiece> pieces, Random random) {
    pieces.add(new fallentowergenerator.Piece(manager, TOWER, pos, rotation));
  ...
  public static class Piece extends SimpleStructurePiece {
    ...
    protected void handleMetadata(String metadata, BlockPos pos, IWorld world, Random random, BlockBox boundingBox) {
      ...
      if (metadata.contains("fallentower_loot")) {
    ...
    public boolean generate(IWorld world, ChunkGenerator<?> generator, Random random, BlockBox box, ChunkPos pos) {
      ...
      if ((world.getBlockState(dirt.up()).isAir() || world.getBlockState(dirt.up()).getBlock().equals(Blocks.GRASS)
          ...
          && world.getBlockState(dirt.up(10)).isAir() && world.getBlockState(dirt.up(10).east(12).south(12)).isAir()) {
